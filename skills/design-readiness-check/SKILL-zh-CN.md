...
description: "检查一个设计是否已经完整到足以进入实现规划（implementation planning）。当设计看起来基本完成，并且需要做最终 readiness review，检查是否缺分支、假设薄弱、风险未解、失败处理不全、验证缺口或非功能性遗漏时使用。在调用 `writing-plans` 之前，或当用户询问当前设计是否可以开始实现时触发。本技能不用于审计外部设计文档，也不替代初始设计工作。"
---
...
- 核心决策仍未解决
- 用户要的是对外部设计文档做通用审计
- design state 中缺少 `design_target_type`

## 工作流 (Workflow)

### 阶段 A：上下文准备 (Phase A: Context Preparation)

目标：加载 design state，并为并行检查组装上下文。

1. 从对话上下文加载当前 `design_state`。
2. 读取设计树内容及所有相关上下文，包括 `open_branches`、`decision_nodes`、`risks`、`validation state`。
3. 如果 `design_state` 不存在，或 design tree 为空，立刻返回 `NOT READY`，并交接给 `design-structure`。
...
目标：使用专用 subagent 并行执行四项独立检查。

7. 启动四个并行 Sonnet subagent，每个 subagent 使用一个 checker agent 模板：
...
**回退机制（Fallback）：** 如果任何 subagent 失败或超时，由主 agent 按对应 agent template 的 rubric 内联完成该项检查。

### 阶段 C：结论综合 (Phase C: Verdict Synthesis)

目标：把各项检查结果合成为明确的 readiness judgment。

9. 把各个 subagent 的 `status` 映射到 readiness checklist：
   - required-state check → “Design target type present”
   - branch-checker → “Design tree present” 与 “Key branches refined”
   - assumption-checker → “Decisions resolved”（如果 assumptions 仍未解决）
...
10. 按 Diagram Conventions 构建一个带 `✓/✗` 的 checklist 图。
...
12. 如果是 `NOT READY`，根据失败项决定交接目标：
   ...
   - assumption-checker 失败 → 交给 `design-refinement`（需要扩充 assumptions）
   ...
   - structural integrity 失败 → 交给 `design-orchestrator`（重新路由所有权）、`design-structure`（派生子树）或 `design-refinement`（收缩重复内联逻辑）
...
14. 返回明确结论与 checklist 图。绝不要说 “probably ready”。

## 就绪标准 (Readiness Standard)

只有满足以下条件，设计才算 ready for planning：
...
用无语言标签的代码块，以状态清单（status checklist）的形式呈现 readiness 结论：
...
- 最大宽度：78 字符
- readiness verdict 中始终包含这份 checklist
