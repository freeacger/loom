...
loom 的图表规范（约 250 行）既存在于 `docs/conventions/terminal-diagram-style.md`，又被内联重复到 design-structure、design-readiness-check、design-refinement、decision-evaluation 等多个技能文件中。mattpocock 通过外部引用（如 `[tests.md](tests.md)`）避免了这个问题。
...
#### 7. 图表规范去重
将内联在多个 SKILL.md 中的 diagram 规则替换为对 `docs/conventions/terminal-diagram-style.md` 的引用，减少维护成本和 token 消耗。
